...
func GetPluginsTotal(field *PluginSearchField) (total int64) {
	...
	if field.Search != "" {
		db = db.Where(
			global.GVA_DB.Where("vul_id like ?", "%"+field.Search+"%").
...
func GetPlugins(page int, pageSize int, field *PluginSearchField) (plugins []Plugin) {
	...
	// 预加载表my_vulnerabilities
	db := global.GVA_DB.Preload("Vulnerability")
...
func GetPlugin(id int) (plugin Plugin) {
	global.GVA_DB.Model(&Plugin{}).Where("id = ?", id).First(&plugin)
	return
}

func EditPlugin(id int, plugin Plugin) bool {
	global.GVA_DB.Model(&Plugin{}).Model(&Plugin{}).Where("id = ?", id).Updates(plugin)
	return true
}

func AddPlugin(plugin Plugin) bool {
	global.GVA_DB.Create(&plugin)
	return true
}

func DeletePlugin(id int) bool {
	global.GVA_DB.Model(&Plugin{}).Where("id = ?", id).Delete(&Plugin{})
	return true
}
...
func ExistPluginByVulId(vul_id string) bool {
	var plugin Plugin
	global.GVA_DB.Model(&Plugin{}).Where("vul_id = ?", vul_id).First(&plugin)
	if plugin.Id > 0 {
		return true
	}
	return false
...
func GenPluginVulId() (string, error) {
	var plugin Plugin
	global.GVA_DB.Model(&Plugin{}).Order("vul_id desc").Limit(1).Select("vul_id").First(&plugin)
	// 数据库没有数据
	if &plugin == nil {
